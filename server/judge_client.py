...
class JudgeClient(object):
    ...
    def _spj(self, in_file_path, user_out_file_path):
        ...
        if result["result"] == sandbox.RESULT_SUCCESS or \
                (result["result"] == sandbox.RESULT_RUNTIME_ERROR and
    ...
    def _judge_one(self, test_case_file_id):
        ...
        if run_result["result"] == sandbox.RESULT_SUCCESS:
            if not os.path.exists(user_output_file):
                ...
                if self._test_case_info.get("spj"):
                    ...
                    if spj_result == SPJ_WA:
                        ...
                        run_result["result"] = sandbox.RESULT_SYSTEM_ERROR
                        run_result["error"] = sandbox.ERROR_SPJ_ERROR
                else:
